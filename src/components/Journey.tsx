...
const events = [
  {
    ...
    description: "Deepening my knowledge in algorithms, data structures, and advanced software engineering."
  },
  {
    year: "2022 – 2024",
    title: "Higher Secondary – Science",
    subtitle: "GVHSS Kottankulangara, Chavara",
    description: "Focused on Physics, Chemistry, and Mathematics with a strong foundation in problem solving."
  },
  {
    year: "2022",
    title: "SSLC",
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative space-y-12">
          ...
          {events.map((event, idx) => (
            <div key={idx} className="journey-item relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 group">
              {/* Dot */}
              ...
              {/* Content Left */}
              <div className={`w-full sm:w-[45%] pl-8 sm:pl-0 ${idx % 2 === 0 ? 'sm:text-right' : 'sm:order-2 sm:text-left'}`}>
                <span className="text-sm font-bold text-accent font-mono mb-2 block">{event.year}</span>
                <h3 className="text-2xl font-bold text-white mb-2">{event.title}</h3>
                <p className="text-lg text-gray-300 font-medium mb-2">{event.subtitle}</p>
                <p className="text-sm text-gray-500 leading-relaxed max-w-sm ml-auto mr-0">{event.description}</p>
              </div>
